    ...
    switch (type) {
      ...
      case 'special': {
        ...
        if (achievement.id === 'special_early_bird') {
          current = myCompletions.filter(c => new Date(c.ts).getHours() < 6).length
        } else if (achievement.id === 'special_night_owl') {
          current = myCompletions.filter(c => new Date(c.ts).getHours() >= 22).length
        ...
        // Perfect day (all due tasks completed on same day)
          ...
          for (const [day, taskIds] of byDay) {
            const myTasksAtThatTime = tasks.filter(t => t.assignee === currentUserId)
            if (taskIds.size >= myTasksAtThatTime.length && myTasksAtThatTime.length > 0) {
              current = 1
              break
            }
          }
        ...
        // Team player
          ...
          const transfers = completions.filter(c => c.userId === currentUserId && c.taskId.startsWith('transfer:to'))
          current = transfers.length
        }
        
        // Area-specific achievements
        else if (achievement.id === 'creative_1') { // Saubermann - alle Putz-Aufgaben in einem Monat
          const cleaningAreas = ['Küche', 'Bad', 'Wohnzimmer', 'Flur']
          const thisMonth = new Date().toISOString().slice(0, 7)
          const monthCompletions = myCompletions.filter(c => new Date(c.ts).toISOString().slice(0, 7) === thisMonth)
        ...
        // Speed achievements
        else if (achievement.id === 'creative_7') { // Schnellschuss - Aufgabe in unter 1 Minute
          // Simplified: if any task was completed, count as 1
          current = myCompletions.length > 0 ? 1 : 0
        } else if (achievement.id === 'creative_8') { // Multitasker - 5 Aufgaben an einem Tag
        ...
        } else if (achievement.id === 'fun_5') { // Speed-Runner - 10 Aufgaben in 1 Stunde
          const sorted = myCompletions.slice().sort((a, b) => a.ts - b.ts)
          let maxInHour = 0
          for (let i = 0; i < sorted.length; i++) {
            const start = sorted[i].ts
            const end = start + 3600000 // 1 hour
            const count = sorted.filter(c => c.ts >= start && c.ts <= end).length
        ...
        else if (achievement.id === 'fun_6') { // Nacht-Schicht - 50 Aufgaben nach 20 Uhr
          current = myCompletions.filter(c => new Date(c.ts).getHours() >= 20).length
        } else if (achievement.id === 'fun_7') { // Morgen-Mensch - 50 Aufgaben vor 8 Uhr
          current = myCompletions.filter(c => new Date(c.ts).getHours() < 8).length
        } else if (achievement.id === 'fun_8') { // Wochenend-Held - 100 Aufgaben am Wochenende
          current = myCompletions.filter(c => {
            const day = new Date(c.ts).getDay()
            return day === 0 || day === 6
          }).length
        ...
        else if (achievement.id === 'time_1') { // 1 Woche dabei
          const daysSince = Math.floor((Date.now() - registrationDate) / (1000 * 60 * 60 * 24))
          current = daysSince
        } else if (achievement.id === 'time_2' || achievement.id === 'time_3' || achievement.id === 'time_4' || achievement.id === 'time_5') {
          const daysSince = Math.floor((Date.now() - registrationDate) / (1000 * 60 * 60 * 24))
          current = daysSince
        }
        ...
        else if (achievement.id === 'creative_4') { // Organisator - 5 Termine im Kalender
          const events = JSON.parse(localStorage.getItem('hh_calendar_events_v1') || '[]')
          current = events.length
        } else if (achievement.id === 'creative_5') { // Rezept-Sammler - 10 Rezepte
          const recipes = JSON.parse(localStorage.getItem('hh_recipes_v1') || '[]')
          current = recipes.length
        } else if (achievement.id === 'creative_6') { // Einkaufs-Profi - 20 Einkaufslisten-Einträge
          const recipes = JSON.parse(localStorage.getItem('hh_recipes_v1') || '[]')
          const totalIngredients = recipes.reduce((sum: number, r: any) => sum + (r.ingredients?.length || 0), 0)
          current = totalIngredients
        }
        
        // Discipline
        else if (achievement.id === 'creative_9') { // Disziplin - Nie eine Aufgabe übersprungen (30 Tage)
          const daysSince = Math.floor((Date.now() - registrationDate) / (1000 * 60 * 60 * 24))
          current = daysSince >= 30 ? 30 : 0 // Simplified
        }
        
        // Community builder
        else if (achievement.id === 'social_1' || achievement.id === 'social_2' || achievement.id === 'social_3') {
          // Simplified: count as 0 for now (would need household member tracking)
          current = 0
        } else if (achievement.id === 'social_4') { // Admin-Helfer
          // Count admin adjustments
          const adminActions = completions.filter(c => c.userId === currentUserId && (c.taskId.startsWith('bonus:') || c.taskId.startsWith('adjust:admin')))
        ...
        // Perfect week
        else if (achievement.id === 'fun_2') { // Perfektionist - 100% in einer Woche
