...
export function WishlistProvider({ children }: { children: React.ReactNode }) {
              ...
              return [
                {
                  ...
                  decidedAt: newItem.decided_at
                    ? new Date(newItem.decided_at).getTime()
                    : undefined,
                ...
                i.id === updated.id
                  ? {
                      ...
                      decidedAt: updated.decided_at
                        ? new Date(updated.decided_at).getTime()
                        : i.decidedAt,
  ...
  const myItems = useMemo(
    () => items.filter((i) => i.createdBy === currentUserId),
    [items, currentUserId]
  );
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.id === id && i.status === "open"
          ? { ...i, status: "assigned", assignedTo: userId }
          : i
      )
    );
  };
  const unassign = (id: string) => {
    setItems((prev) =>
      prev.map((i) =>
        i.id === id && i.status === "assigned"
          ? { ...i, status: "open", assignedTo: null }
          : i
      )
    );
  };
  const redeem = (id: string, userId: string) => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.id === id ? { ...i, status: "redeemed", decidedAt: Date.now() } : i
      )
    );
    return true;
  };
  const reject = (id: string) =>
    setItems((prev) =>
      prev.map((i) =>
        i.id === id ? { ...i, status: "rejected", decidedAt: Date.now() } : i
      )
    );
