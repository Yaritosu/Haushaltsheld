...
export default function CalendarPage({ onLogout }: Props) {
  ...
  const eventsByDate = useMemo(() => {
    ...
    for (const t of tasks) {
      if (isDueNow(t, today)) {
        ...
        map[todayStr].push({ id: `t-${t.id}`, title: t.title, date: todayStr, type: 'task' })
      }
    }
    return map
  ...
  return (
    <AppShell onLogout={onLogout}>
      <div className="dashboard-card" style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', alignItems: 'center' }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', fontWeight: 700, fontSize: '1.1rem' }}>{monthName}</div>
          <button className="nav-btn" onClick={() => {
            let m = month + 1, y = year
            if (m > 12) { m = 1; y++; }
            setViewDate(`${y}-${String(m).padStart(2,'0')}`)
        ...
        <div style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.5rem' }}>
            ...
            return (
              <div 
                ...
                style={{
                  minHeight: 80,
                  padding: '0.5rem',
                  background: day ? (isToday ? 'rgba(107, 231, 107, 0.15)' : 'rgba(255,255,255,0.05)') : 'transparent',
                  borderRadius: 8,
                  border: isToday ? '2px solid rgba(107, 231, 107, 0.4)' : '1px solid rgba(255,255,255,0.08)',
                  display: 'flex',
              ...
              >
                {day && (
                  <>
                    <div style={{ fontWeight: 700, fontSize: '0.9rem', color: isToday ? '#6be76b' : '#fff' }}>{day}</div>
                    {evs.map(ev => (
                      <div key={ev.id} className="muted" style={{ fontSize: '0.75rem', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          ...
          <div className="task-list" style={{ marginTop: '0.75rem' }}>
            {events.map(ev => (
              <div key={ev.id} className="task-item">
                ...
                <button className="task-check-btn" onClick={() => handleDelete(ev.id)} title="Löschen" style={{ background: 'rgba(255,100,100,0.2)' }}>
                  <TrashIcon style={{ width: 16, height: 16 }} />
                </button>
              </div>
            ))}
            {events.length === 0 && (
              <div className="task-item"><div className="muted">Noch keine Termine</div></div>
            )}
          </div>
        </div>
      </div>
