...
import { ClipboardDocumentListIcon, CheckIcon, PencilIcon, TrashIcon, UserPlusIcon } from '@heroicons/react/24/outline'
import { useMemo, useState } from 'react'
import { useTasks, ALL_AREAS, RECURRENCE_LABEL, RECURRENCE_ORDER, type Area, type Recurrence, type Task } from '../context/TasksContext'
import { useHousehold } from '../context/HouseholdContext'

 type Props = { onLogout: () => void };

 export default function TasksPage({ onLogout }: Props) {
  const { tasks, setTasks, currentUserId, addTask, assignToMe, unassign, toggleDone, isDoneForNow, isDueNow } = useTasks()
  ...
  const isAdmin = membership?.role === 'admin'
   ...
   const handleCreate = () => {
     ...
     if (editingId) {
       ...
       setTasks(prev => prev.map(t => t.id === editingId ? { ...t, title: title.trim(), points, area, recurrence } : t))
       setEditingId(null)
     } else {
   ...
   const handleComplete = (taskId: string) => {
     if (!confirm('Aufgabe endgültig als abgeschlossen markieren (einmalig/sonder)?')) return
     setTasks(prev => prev.filter(t => t.id !== taskId))
   }
   ...
   const handleAssignToOther = (taskId: string) => {
     ...
     if (!userId) return
     setTasks(prev => prev.map(t => t.id === taskId ? { ...t, assignee: userId } : t))
   }
   ...
   return (
     <AppShell onLogout={onLogout}>
       <div className="dashboard-card" style={{ maxWidth: 1000, margin: '0 auto' }}>
         ...
         {showForm && (
           <div className="task-item" style={{ marginTop: '1rem', flexDirection: 'column', gap: '0.75rem' }}>
             <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '0.5rem', width: '100%' }}>
             ...
             <div style={{ display: 'flex', gap: '0.5rem' }}>
               <button className="card-action-btn" onClick={handleCreate}>{editingId ? 'Aktualisieren' : 'Speichern'}</button>
               <button className="card-action-btn secondary" onClick={() => { setShowForm(false); setEditingId(null); }}>Abbrechen</button>
             </div>
           </div>
         )}
                     ...
                     return (
                       <div key={t.id} className="task-item" style={{ opacity: done ? 0.7 : 1, flexWrap: 'wrap' }}>
                         ...
                         <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                           ...
                           <button className="task-check-btn" onClick={() => handleEdit(t)} title="Bearbeiten">
                             <PencilIcon style={{ width: 16, height: 16 }} />
                           </button>
                           {isAdmin && (
                             <button className="task-check-btn" onClick={() => handleAssignToOther(t.id)} title="Anderen zuweisen (Admin)">
                               <UserPlusIcon style={{ width: 16, height: 16 }} />
                             </button>
                           )}
                           <button className="task-check-btn" onClick={() => handleDelete(t.id)} title="Löschen" style={{ background: 'rgba(255,100,100,0.2)' }}>
                             <TrashIcon style={{ width: 16, height: 16 }} />
                           </button>
                           {t.assignee === currentUserId ? (
                             <button className="card-action-btn secondary" style={{ width: 'auto', padding: '0.4rem 0.75rem' }} onClick={() => unassign(t.id)}>Zuweisung entfernen</button>
                           ) : !t.assignee && (
