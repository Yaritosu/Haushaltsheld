...
export default function InvitePage() {
  ...
  const [status, setStatus] = useState<'idle' | 'joining' | 'done' | 'error' | 'needs-login'>('idle')
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!SUPABASE_CONFIGURED || !supabase) {
        setStatus('error')
        setMessage('Supabase ist nicht konfiguriert. Einladungen sind deaktiviert.')
        return
      }
      ...
      try {
        setStatus('joining')
        const { error } = await supabase.rpc('join_household_by_code', { code, user_id: user.id })
        if (error) {
  ...
  return (
    <div className="app-root login-page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="card" style={{ maxWidth: 520, width: '100%' }}>
        ...
        <p className="muted" style={{ textAlign: 'center' }}>Code: <strong>{code}</strong></p>
        <div style={{ textAlign: 'center', marginTop: 12 }}>
          ...
          {status === 'error' && (
            <>
              <div className="form-error" style={{ display: 'inline-block' }}>{message}</div>
