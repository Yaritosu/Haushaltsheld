...
import { CakeIcon, PlusIcon, TrashIcon, ShoppingCartIcon, PencilIcon } from '@heroicons/react/24/outline';
...
export default function RecipesPage({ onLogout }: Props) {
  ...
  const [mealPlan, setMealPlan] = useState<MealPlan[]>(() => {
    ...
    return ['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag', 'Sonntag'].map(d => ({ day: d }))
  })
  ...
  const handleAssignMeal = (day: string, recipeId: string) => {
    ...
    const updated = mealPlan.map(m => m.day === day ? { day, recipeId, recipeName: r?.name } : m)
    setMealPlan(updated)
    ...
    <AppShell onLogout={onLogout}>
      <div className="dashboard-grid">
        ...
        <div className="dashboard-card">
          <div className="card-icon"><CakeIcon style={{ width: 28, height: 28 }} /></div>
          <h3>Rezepte</h3>
          ...
          <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              {editingId && (
                <button className="card-action-btn secondary" onClick={() => { setEditingId(null); setNewName(''); setNewIngredients(''); setNewInstructions(''); }}>
                  Abbrechen
                </button>
              )}
            </div>
          ...
          <div className="task-list" style={{ marginTop: '1rem' }}>
            {recipes.map(r => (
              <div key={r.id} className="task-item" style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', width: '100%', alignItems: 'center' }}>
                  <div className="task-title" style={{ flex: 1 }}>{r.name}</div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="task-check-btn" onClick={() => handleEditRecipe(r)} title="Bearbeiten">
                      <PencilIcon style={{ width: 16, height: 16 }} />
                    </button>
                    <button className="task-check-btn" onClick={() => handleDeleteRecipe(r.id)} title="Löschen" style={{ background: 'rgba(255,100,100,0.2)' }}>
            ...
            {recipes.length === 0 && (
              <div className="task-item"><div className="muted">Noch keine Rezepte</div></div>
            )}
          </div>
        </div>
        ...
        {/* Essensplan */}
        <div className="dashboard-card">
          <div className="card-icon"><ShoppingCartIcon style={{ width: 28, height: 28 }} /></div>
          <h3>Wochenplan</h3>
          <p className="muted">Plane deine Mahlzeiten für die Woche</p>

          <div className="task-list" style={{ marginTop: '1rem' }}>
            {mealPlan.map(m => (
              <div key={m.day} className="task-item" style={{ alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <div className="task-title">{m.day}</div>
                  <div className="task-meta muted">{m.recipeName || 'Noch kein Rezept'}</div>
                </div>
                <select 
        ...
        <div className="dashboard-card">
          ...
          <div className="task-list" style={{ marginTop: '1rem' }}>
            ...
            {shoppingList.length === 0 && (
              <div className="task-item"><div className="muted">Keine Einträge (Plane Mahlzeiten)</div></div>
            )}
          </div>
        </div>
      </div>
