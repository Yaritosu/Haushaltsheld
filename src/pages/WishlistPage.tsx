...
export default function WishlistPage({ onLogout }: Props) {
  ...
  const onAdd = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      setItems((prev) =>
        prev.map((i) =>
          i.id === editingId ? { ...i, title: title.trim(), points } : i
        )
      );
      setEditingId(null);
    } else {
  ...
  const visibleItems = useMemo(
    () => items.filter((i) => i.status !== "redeemed"),
    [items]
  );
  ...
  return (
    <AppShell onLogout={onLogout}>
      ...
      >
        ...
        <h3>Wunschliste</h3>
        <p className="muted">
          Mitglieder können Wünsche anlegen. Admins können freigeben und Punkte
          vergeben.
        </p>
        ...
        >
              ...
              >
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <div className="task-meta muted">
                    {it.points} P • {it.status}
                    {it.assignedTo && it.status !== "redeemed"
                      ? ` • zugeordnet`
                      : ""}
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {it.status === "open" && (
                    <button
                      className="nav-btn"
                      onClick={() => assignTo(it.id, currentUserId)}
                    >
                      Ich will das
                    </button>
                  )}
                    ...
                    it.assignedTo === currentUserId && (
                      <>
                        ...
                        <button
                          className="nav-btn"
                          disabled={!canRedeem}
                          onClick={() => redeem(it.id, currentUserId)}
                          title={canRedeem ? "Einlösen" : "Nicht genug Punkte"}
                        >
                          Einlösen
                        </button>
                      </>
                  ...
                  {isMine && it.status === "open" && (
                    <>
                      <button
                        ...
                        onClick={() => handleEdit(it.id, it.title, it.points)}
                        title="Bearbeiten"
                      >
          ...
          {visibleItems.length === 0 && (
            <div className="task-item">
              <div className="muted">Noch keine Wünsche</div>
            </div>
          )}
        </div>
      </div>
