...
import { TrophyIcon, ChartBarIcon, LockClosedIcon } from '@heroicons/react/24/outline'
...
export function AchievementStatsPage() {
  ...
  const nextUnlockable = useMemo(() => {
    const candidates: Array<{ id: string; title: string; progress: number; target: number; icon: string }> = []
    ACHIEVEMENTS.forEach(ach => {
    ...
    return candidates.sort((a, b) => (b.progress / b.target) - (a.progress / a.target)).slice(0, 5)
    ...
    <div className="page-container">
      ...
      <div className="dashboard-card" style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '1rem', marginBottom: '0.5rem', color: 'var(--bronze-darker)' }}>
              {unlockedCount} von {totalAchievements} Achievements freigeschaltet
            </div>
            <div style={{ 
              width: '100%', 
              height: '24px', 
      ...
      {nextUnlockable.length > 0 && (
        <div className="dashboard-card" style={{ marginBottom: '2rem' }}>
          <h2 style={{ fontSize: '1.25rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <LockClosedIcon className="icon" style={{ width: 24, height: 24 }} />
            Nächste erreichbare Achievements
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {nextUnlockable.map(n => {
              const IconComponent = require('@heroicons/react/24/outline')[n.icon]
              const percent = Math.round((n.progress / n.target) * 100)
              return (
                <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  {IconComponent && <IconComponent className="icon" style={{ width: 32, height: 32, flexShrink: 0 }} />}
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.95rem', fontWeight: 500, marginBottom: '0.25rem' }}>{n.title}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <span style={{ fontSize: '0.85rem', color: 'var(--bronze-darker)', whiteSpace: 'nowrap' }}>
                        {n.progress} / {n.target}
                      </span>
                    </div>
                  </div>
      ...
      <div className="dashboard-card">
        <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem' }}>Fortschritt nach Kategorie</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {Object.entries(categories).map(([cat, achs]) => {
            const unlockedInCat = achs.filter(a => unlocked.includes(a.id)).length
            ...
            return (
              <div key={cat}>
                <div style={{ 
                  ...
                  <span style={{ fontSize: '0.9rem', color: 'var(--bronze-darker)' }}>
